...
describe('FCB', function () {
  ...
  const FEE = 4
  const DECIMAL = '18'
  const CAP = ethers.utils.parseUnits('10000', DECIMAL);
  ...
  beforeEach(async function () {
    ...
    this.token = await Token.deploy(beneficiaryAddress, CAP);
...
////sell
  ...
  //  )
  //});
  //it("mint over cap", async function () {
  //  const cap = await this.token.cap()
  //  await expect(this.token.mint(this.owner.address, cap.add(1))).to.be.revertedWith("ERC20Capped: cap exceeded")
  //  await expect(
  //    this.token
  //    .connect(this.accounts[1])
  //    .mint(this.accounts[1].address, cap)
